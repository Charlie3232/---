...
function handleStatusChange() {
  ...
  if (status === "已解決" || status === "Done") {
    const closedDate = prompt("此項目已完成！請輸入實際結案日期 (YYYY-MM-DD):", get2026Date());
    if (closedDate) {
...
function openModal(type) {
  ...
  // 預設日期 2026
  document.getElementById('input-deadline').value = get2026Date();
...
function openEdit(id) {
  ...
  document.getElementById('input-deadline').value = i.deadline ? i.deadline.replace(/\//g, '-') : get2026Date();
