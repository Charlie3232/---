...
function handleLogin() {
  ...
  if (userList.length === 0) { alert("正在連線伺服器..."); return; }
  const user = userList.find(u => u.id === inputID && u.pwd === pwdInput);
  if (user) {
...
function handleStatusChange() {
  ...
  if (status === "已解決" || status === "Done") {
    const closedDate = prompt("專案完成！請輸入實際結案日期 (YYYY-MM-DD):", new Date().toISOString().split('T')[0]);
...
function renderStats() {
  ...
  const fProds = getCheckedValues('stats-product');
  ...
  allIssues.forEach(i => {
    ...
    if(fProds.length > 0 && !fProds.includes(String(i.product))) return;
  ...
  if(totalIssues === 0) { container.innerHTML = "<p style='color:#888;'>此區間內無紀錄。</p>"; return; }
  ...
  container.innerHTML = Object.keys(ownerCounts).sort((a,b) => ownerCounts[b] - ownerCounts[a]).map((owner, idx) => {
    ...
    return `
      <div class="stat-row">
        ...
        <div class="stat-value">${count} 件 (${pct}%)</div></div>
      </div>
    `;
  }).join('');
}
...
function renderIssues() {
  ...
  let filtered = allIssues.filter(i => 
    ...
    (fStats.length === 0 ? (String(i.status) !== "已解決") : fStats.includes(String(i.status))) &&
